...
min_lectures = st.sidebar.slider("Minimum Number of Lectures", 0, int(df['num_published_lectures'].max()), 0, 1)
...
@st.cache_data
def load_data():
    df = pd.read_csv('udemy_output_All_Finance_-_Accounting_p1_p626.csv')
    df = df.dropna()
    if 'is_wishlisted' in df.columns:
        df = pd.get_dummies(df, columns=['is_wishlisted'], drop_first=True)
    # Extract year from 'published_time'
    df['published_year'] = pd.to_datetime(df['published_time']).dt.year
    return df
...
with col1:
    ...
    st.write(f"**Mean Squared Error**<br>{mse:,.2f}", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
with col2:
    st.markdown('<div class="metric-box">', unsafe_allow_html=True)
    st.write(f"**R-squared**<br>{r2:.2f}", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
